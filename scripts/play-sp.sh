...
if [ "$3" = jo ]; then
    ...
    grep -Fxq 'PASS: kejim_post' "$package/smoke-jo-result.txt"
    grep -Fq 'PASS: JO opening, wheels, weapon switching, save/load, and Kejim transition' "$package/jo-mvp-result.txt"
fi
printf '%s\n' "$package"
REMOTE
