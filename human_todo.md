...
- [x] User approved capsule wall occlusion. Wall mode now defaults to `1`; other capsule defaults are retained.
- [ ] Check detached hands, arms, heads, and the remaining body after the surface-tree capsule fix.
- [x] User kept capsule softness `1`, radius `1`, and range `64` after calibration.
...
- [ ] Compare `r_sss 0` and `1` at `r_sssRadius 0.5` on the added humanoids. Check texture detail, sharp specular highlights, boundaries, and overlap with smoke or refractive effects.
- [x] User approved SSS strength `1` and radius `0.5`. These are now the defaults.
- [ ] Check expanded humanoid skin masks, especially partial masks, gloves, and custom outfits. Use `r_sssDebug 1` to find missing skin or unwanted clothing coverage.
...
- [ ] Replay the Kejim Base CCTV cinematic from an earlier save. Check Galak, the officer's gait, and the transition to the next scene.
...
- [ ] Replay Artus Topside. Check Kyle's facing and poses, Desann's attacks, the defeat scene, and both aftermath sabers. Test skipping separately.
- [ ] Continue through the shrine and Yavin Trial ending without skipping. Check the dialogue, movement, and return of player control.
