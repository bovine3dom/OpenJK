...
static void CG_DrawCrosshair( vec3_t worldPoint )
{
	...
	else if ( cg_crosshairIdentifyTarget.integer )
	{
		...
		{
			// Destructible objects are blue. because yellow was too hard to see
			ecolor[0] = 0.0;//R
			ecolor[1] = 0.0;//G
			ecolor[2] = 1.0;//B
		}
...
static void CG_Draw2D( void )
{
	...
	CG_UpdateForceWheel((qboolean)(wheelAllowed && !cg.zoomMode));
	CG_UpdateWeaponWheel((qboolean)(wheelAllowed && !(g_entities[0].flags & FL_LOCK_PLAYER_WEAPONS)));
