...
{
	if ( !d_squadTactics->integer || !missile->owner || !missile->owner->inuse || !missile->owner->client
		|| missile->damage <= 0 || missile->splashDamage || missile->s.pos.trType != TR_LINEAR )
		return;
	vec3_t direction, mins, maxs;
	float radius = Com_Clamp( 0, 256, g_squadPressureRadius->value );
	if ( radius <= 0 )
		return;
	VectorSubtract( end, start, direction );
	float lengthSquared = VectorLengthSquared( direction );
	if ( lengthSquared < 1 )
		return;
	for ( int axis = 0; axis < 3; ++axis )
	{
		mins[axis] = Q_min( start[axis], end[axis] ) - radius;
		maxs[axis] = Q_max( start[axis], end[axis] ) + radius;
	}
	gentity_t *nearby[MAX_GENTITIES];
	int count = gi.EntitiesInBox( mins, maxs, nearby, MAX_GENTITIES );
	for ( int i = 0; i < count; ++i )
	{
		gentity_t *self = nearby[i];
		if ( !self->NPC || !self->client || !self->enemy || self == missile->owner
			|| missile->owner->client->playerTeam != self->client->enemyTeam
			|| !AI_ValidateTacticalMember( self->NPC->group, self )
			|| !TIMER_Done( self, "pressureDebounce" ) )
			continue;
		vec3_t chest, offset, closest;
		VectorCopy( self->currentOrigin, chest );
		chest[2] += (self->mins[2]+self->maxs[2])*0.5f;
		VectorSubtract( chest, start, offset );
		float fraction = Com_Clamp( 0, 1, DotProduct( offset, direction )/lengthSquared );
		VectorMA( start, fraction, direction, closest );
		if ( DistanceSquared( closest, chest ) > radius*radius )
			continue;
		if ( !G_ClearLOS( self, closest, chest ) )
		{
			if ( TIMER_Done( self, "pressureWallTrace" ) )
			{
				TIMER_Set( self, "pressureWallTrace", 500 );
				Debug_Printf( debugNPCAI, DEBUG_LEVEL_DETAIL, "squad event=pressure_ignored ent=%d reason=wall\n", self->s.number );
			}
			continue;
		}
		// This is local pressure, not a sight or target-acquisition report.
		TIMER_Set( self, "incomingFire", 1500 );
		TIMER_Set( self, "underFire", Q_max( 2000, TIMER_Get( self, "underFire" )-level.time ) );
		TIMER_Set( self, "pressureDebounce", 300 );
		Debug_Printf( debugNPCAI, DEBUG_LEVEL_INFO, "squad event=incoming_fire ent=%d missile=%d time=%d distance=%.1f\n",
			self->s.number, missile->s.number, level.time, Distance( closest, chest ) );
	}
}

static void ST_SaberPressure( gentity_t *self, qboolean visible )
{
	if ( !visible || !d_squadTactics->integer || !self->enemy || !self->enemy->client
		|| self->enemy->client->ps.weapon != WP_SABER || !self->enemy->client->ps.SaberActive()
		|| self->enemy->client->ps.saberInFlight || !TIMER_Done( self, "pressureDebounce" ) )
		return;
	switch ( self->client->ps.weapon )
	{
	case WP_NONE: case WP_MELEE: case WP_SABER: case WP_STUN_BATON:
	case WP_TUSKEN_STAFF: case WP_NOGHRI_STICK:
		return;
	default:
		break;
	}
	if ( !AI_ValidateTacticalMember( self->NPC->group, self ) )
		return;
	float radius = Com_Clamp( 0, 512, g_squadSaberPressureRadius->value );
	float distance = DistanceSquared( self->currentOrigin, self->enemy->currentOrigin );
	if ( radius <= 0 || distance > radius*radius )
		return;
	// The caller has just confirmed sight. Proximity supplies pressure, not a new target.
	TIMER_Set( self, "incomingFire", 1500 );
	TIMER_Set( self, "saberPressure", 1500 );
	TIMER_Set( self, "underFire", Q_max( 2000, TIMER_Get( self, "underFire" )-level.time ) );
	TIMER_Set( self, "pressureDebounce", 300 );
	Debug_Printf( debugNPCAI, DEBUG_LEVEL_INFO, "squad event=saber_pressure ent=%d enemy=%d time=%d distance=%.1f\n",
		self->s.number, self->enemy->s.number, level.time, sqrtf(distance) );
}

qboolean ST_PressureRun( gentity_t *self )
{
	return self && self->NPC && self->NPC->tacticRole == 1 && g_squadPressureOverrides->integer
		&& TIMER_Exists( self, "pressureMove" ) && AI_ValidateTacticalMember( self->NPC->group, self ) ? qtrue : qfalse;
}
...
static qboolean ST_Move( void )
{
	...
	const bool runRetreat = NPCInfo->tacticRole == 1
		...
		&& (!(NPCInfo->scriptFlags & SCF_WALKING) || (NPCInfo->scriptFlags & SCF_RUNNING) || ST_PressureRun( NPC ));
...
{
	if ( NPCInfo->tacticRole == 2 && !TIMER_Exists( NPC, "coverPeek" )
...
static void ST_ReportNearby( AIGroupInfo_t *group )
{
	...
	for ( int offset = 0; offset < group->numGroup; offset++ )
	{
		...
		if ( AI_ValidateGroupMember( group, member, qtrue ) && member->enemy == enemy
			&& TIMER_Get( member, "squadSight" ) == observed
...
{
	if ( !AI_CanReport(self) || !self->enemy || !self->enemy->inuse || !self->enemy->client
		|| self->enemy->health <= 0 || (self->enemy->flags & FL_NOTARGET)
		|| self->enemy->client->playerTeam == self->client->playerTeam )
		return;
	qboolean visible = G_ClearLOS( self, self->enemy );
	if ( visible )
	{
		self->NPC->enemyLastSeenTime = level.time;
		VectorCopy( self->enemy->currentOrigin, self->NPC->enemyLastSeenLocation );
		TIMER_Set( self, "squadSight", 0 );
	}
	AI_GetGroup( self );
	...
	if ( !group || group->enemy != self->enemy || !AI_ValidateGroupMember(group, self, qtrue) )
		return;
	if ( visible )
		AI_GroupUpdateEnemyLastSeen( group, self->enemy->currentOrigin );
...
void ST_ClearTactic( gentity_t *self, const char *reason )
{
	...
	TIMER_Remove( self, "pressureMove" );
	TIMER_Remove( self, "coverReturn" );
	if ( TIMER_Exists( self, "saberEscape" ) )
		TIMER_Remove( self, "SafeToGoToDURATION" );
	TIMER_Remove( self, "saberEscape" );
	TIMER_Remove( self, "coverPeek" );
	TIMER_Remove( self, "coverPair" );
	TIMER_Remove( self, "coverPaired" );
	...
	if ( (role == 3 || role == 4) && info->group )
	{
		for ( int i = 0; i < info->group->numGroup; i++ )
		{
			...
			if ( other->NPC && other->NPC->tacticRole == 5 && !TIMER_Exists( other, "coverSupport" ) )
...
static qboolean ST_CoverProtects( gentity_t *self, const vec3_t point, const vec3_t known, float tolerance = 16 )
{
	vec3_t threatHead, firing;
	VectorCopy( known, threatHead );
	threatHead[2] += 40;
	for ( int side = 0; side < 12; ++side )
	{
		VectorCopy( point, firing );
		firing[0] += side & 1 ? self->maxs[0]+tolerance : self->mins[0]-tolerance;
		firing[1] += side & 2 ? self->maxs[1]+tolerance : self->mins[1]-tolerance;
		float low = self->client->crouchheight*0.5f;
		firing[2] += low + (self->client->standheight-low)*(side/4)*0.5f;
		for ( int height = 0; height < 2; ++height )
		{
			trace_t trace;
			gi.trace( &trace, firing, NULL, NULL, height ? threatHead : known,
				self->s.number, CONTENTS_OPAQUE, (EG2_Collision)0, 0 );
			// Check the body and arrival tolerance. Doors cannot supply reliable cover.
			if ( trace.startsolid || trace.allsolid || trace.fraction == 1.0f || trace.entityNum != ENTITYNUM_WORLD )
				return qfalse;
		}
	}
	return qtrue;
...
static qboolean ST_DirectEscape( gentity_t *self, const vec3_t start, const vec3_t goal, const vec3_t known )
{
	vec3_t step, away, bounds;
	VectorSubtract( goal, start, step );
	VectorSubtract( start, known, away );
	if ( DotProduct( step, away ) < 0 )
		return qfalse;
	VectorCopy( self->maxs, bounds );
	bounds[2] = self->client->standheight;
	trace_t trace;
	gi.trace( &trace, start, self->mins, bounds, goal, self->s.number, self->clipmask, (EG2_Collision)0, 0 );
	if ( trace.startsolid || trace.allsolid || trace.fraction < 1 )
		return qfalse;
	int steps = Q_max( 1, (int)ceilf(VectorLength(step)/24) );
	for ( int i = 1; i <= steps; ++i )
	{
		vec3_t top, bottom;
		VectorMA( start, (float)i/steps, step, top );
		VectorCopy( top, bottom );
		bottom[2] -= 24;
		gi.trace( &trace, top, self->mins, bounds, bottom, self->s.number, self->clipmask, (EG2_Collision)0, 0 );
		if ( trace.startsolid || trace.allsolid || trace.fraction == 1 || trace.plane.normal[2] < 0.7f )
			return qfalse;
	}
	return qtrue;
}
...
static qboolean ST_CyclePosition( gentity_t *self, const vec3_t known, qboolean cover, vec3_t goal, int &cp, qboolean peek = qfalse, const float *from = NULL, qboolean requirePeek = qtrue, float retreatDistance = 0 )
{
	if ( !self->NPC->tempGoal || NAV::GetNearestNode( self ) == WAYPOINT_NONE )
		return qfalse;
	vec3_t origin;
	VectorCopy( from ? from : self->currentOrigin, origin );
	cp = peek ? -1 : NPC_FindCombatPoint( origin, known, origin,
		(retreatDistance > 0 ? CP_RETREAT : cover ? CP_COVER : CP_CLEAR)|CP_AVOID_ENEMY|CP_HAS_ROUTE,
		128, self->NPC->lastFailedCombatPoint, known );
	NAV::TNodeHandle nodes[64];
	int count = peek ? 32 : NAV::GetNearbyGroundNodes( origin, nodes, 64 );
	int nodeCount = count;
	if ( cover )
		count += 128;
	vec3_t muzzleOffset;
	...
	for ( int i = -1; i < count; ++i )
	{
		...
		if ( peek )
		{
			float angle = (i%8)*M_PI/4;
			VectorCopy( origin, point );
			point[0] += cosf(angle)*(48 + 32*(i/8));
			point[1] += sinf(angle)*(48 + 32*(i/8));
			vec3_t top, bottom, bounds;
			VectorCopy( self->maxs, bounds );
			bounds[2] = self->client->standheight;
			trace_t step;
			gi.trace( &step, origin, self->mins, bounds, point, self->s.number, self->clipmask, (EG2_Collision)0, 0 );
			if ( step.startsolid || step.allsolid || step.fraction < 1 )
				continue;
			bool supported = true;
			for ( int part = 1; part <= 8 && supported; ++part )
			{
				VectorSubtract( point, origin, top );
				VectorMA( origin, part/8.0f, top, top );
				VectorCopy( top, bottom );
				bottom[2] -= 24;
				gi.trace( &step, top, self->mins, bounds, bottom, self->s.number, self->clipmask, (EG2_Collision)0, 0 );
				supported = !step.startsolid && !step.allsolid && step.fraction < 1 && step.plane.normal[2] >= 0.7f;
			}
			if ( !supported )
				continue;
		}
		else if ( cover && i >= nodeCount )
		{
			int sample = i-nodeCount;
			float angle = (sample%16)*M_PI/8;
			VectorCopy( origin, point );
			point[0] += cosf(angle)*(64+32*(sample/16));
			point[1] += sinf(angle)*(64+32*(sample/16));
			vec3_t top, bottom;
			VectorCopy( point, top );
			VectorCopy( point, bottom );
			top[2] += 24;
			bottom[2] -= 24;
			trace_t floor;
			gi.trace( &floor, top, self->mins, self->maxs, bottom, self->s.number, self->clipmask, (EG2_Collision)0, 0 );
			if ( floor.startsolid || floor.allsolid || floor.fraction == 1 || floor.plane.normal[2] < 0.7f )
				continue;
			VectorCopy( floor.endpos, point );
		}
		else
		...
		for ( int number = 0; number < globals.num_entities && !claimed; ++number )
		{
			...
			if ( other != self && other->inuse && other->NPC
				&& (((other->NPC->tacticRole == 1 || other->NPC->tacticRole == 3)
					...
					|| (TIMER_Exists( other, "coverPair" ) && DistanceSquared( point, other->NPC->tacticCover ) < 48*48)) )
				claimed = true;
		}
		if ( claimed )
			continue;
		...
		if ( peek )
		{
			// Check the standing muzzle while facing the recorded threat.
			vec3_t direction;
			VectorSubtract( known, point, direction );
			direction[2] = 0;
			VectorNormalize( direction );
			VectorMA( point, 12, direction, firing );
			firing[2] += self->client->standheight - 8;
		}
		if ( retreatDistance <= 0 && G_ClearLOS( self, firing, known ) == cover )
			continue;
		if ( cover && retreatDistance <= 0 && !ST_CoverProtects( self, point, known ) )
			continue;
		trace_t trace;
		...
		if ( trace.startsolid || trace.allsolid || NAV::GetNearestNode( point ) == WAYPOINT_NONE
			...
			|| (retreatDistance > 0 ? !ST_DirectEscape( self, origin, point, known )
				: !NAV::SafePathExists( origin, point, known, 128*128, self )) )
			continue;
		if ( cover && requirePeek )
		{
			vec3_t peekGoal;
			int peekCP;
...
static qboolean ST_PeekCover( gentity_t *self )
{
	auto *info = self->NPC;
	if ( !TIMER_Exists( self, "coverPair" ) )
		return qfalse;
	vec3_t known;
	if ( !(info->scriptFlags & SCF_CHASE_ENEMIES)
		|| (info->scriptFlags & (SCF_DONT_FLEE|SCF_DONT_FIRE|SCF_FIRE_WEAPON)) )
	{
		ST_ClearTactic( self, "cover_invalid" );
		return qtrue;
	}
	if ( TIMER_Done( self, "coverRecheck" ) && ST_GetLastSeenPosition( self, known ) )
	{
		TIMER_Set( self, "coverRecheck", 500 );
		if ( DistanceSquared( info->tacticCover, known ) < 128*128
			|| !ST_CoverProtects( self, info->tacticCover, known, 0 )
			|| (info->tacticRole == 1 && DistanceSquared( known, info->tacticThreat ) > 64*64) )
		{
			ST_ClearTactic( self, "cover_exposed" );
			TIMER_Set( self, "coverRelocate", 3000 );
			return qtrue;
		}
		if ( info->tacticRole == 2 )
			VectorCopy( known, info->tacticThreat );
	}
	bool peeking = TIMER_Exists( self, "coverPeek" );
	bool pressure = !TIMER_Done( self, "incomingFire" ) || !TIMER_Done( self, "underFire" );
	bool blocked = info->tacticRole == 2 && TIMER_Exists( self, "peekBlocked" ) && TIMER_Done( self, "peekBlocked" );
	if ( peeking && blocked )
	{
		if ( TIMER_Exists( self, "peekBlockedOnce" ) )
		{
			ST_ClearTactic( self, "firing_lane_blocked" );
			TIMER_Set( self, "coverRelocate", 3000 );
			return qtrue;
		}
		TIMER_Set( self, "peekBlockedOnce", 15000 );
	}
	if ( peeking && (pressure || self->health*2 < self->max_health || TIMER_Done( self, "coverPair" )
		|| (info->tacticRole == 2 && (info->tacticDeadline <= level.time || blocked))) )
	{
		trace_t trace;
		gi.trace( &trace, self->currentOrigin, self->mins, self->maxs, info->tacticCover,
			self->s.number, self->clipmask, (EG2_Collision)0, 0 );
		if ( trace.startsolid || trace.allsolid || trace.fraction < 1
			|| !NAV::SafePathExists( self->currentOrigin, info->tacticCover, info->tacticThreat, 128*128, self ) )
		{
			ST_ClearTactic( self, "cover_blocked" );
			TIMER_Set( self, "coverRelocate", 3000 );
			return qtrue;
		}
		TIMER_Remove( self, "coverPeek" );
		TIMER_Remove( self, "peekBlocked" );
		NPC_SetMoveGoal( self, info->tacticCover, 8, qtrue, info->tacticCP );
		ST_AssignTactic( self, 1, info->tacticCP, info->tacticThreat, info->tacticCover );
		Debug_Printf( debugNPCAI, DEBUG_LEVEL_INFO, "squad event=peek_withdraw ent=%d pressure=%d\n", self->s.number, pressure );
		return qtrue;
	}
	if ( info->tacticRole != 2 || peeking )
		return qfalse;
	if ( TIMER_Done( self, "coverPair" ) )
	{
		TIMER_Remove( self, "coverPair" );
		return qfalse;
	}
	if ( pressure )
		info->tacticDeadline = Q_max( info->tacticDeadline, level.time+1500 );
	if ( info->tacticDeadline > level.time )
		return qtrue;
	vec3_t goal;
	int cp = -1;
	if ( self->health*2 < self->max_health || !ST_CyclePosition( self, info->tacticThreat, qfalse, goal, cp, qtrue, info->tacticCover ) )
	{
		TIMER_Remove( self, "coverPair" );
		return qfalse;
	}
	NPC_SetMoveGoal( self, goal, 8, qtrue, info->tacticCP );
	ST_AssignTactic( self, 1, info->tacticCP, info->tacticThreat, goal );
	TIMER_Set( self, "coverPeek", 6000 );
	Debug_Printf( debugNPCAI, DEBUG_LEVEL_INFO, "squad event=peek_move ent=%d anchor=%.3f,%.3f,%.3f goal=%.3f,%.3f,%.3f\n", self->s.number,
		info->tacticCover[0], info->tacticCover[1], info->tacticCover[2], goal[0], goal[1], goal[2] );
	return qtrue;
}

static gentity_t *ST_RetreatingMember( gentity_t *self, AIGroupInfo_t *group )
{
	for ( int i = 0; i < group->numGroup; ++i )
	{
		gentity_t *other = &g_entities[group->member[i].number];
		if ( other != self && AI_ValidateTacticalMember( group, other ) && other->enemy == self->enemy
			&& other->NPC->tacticRole == 1 && !TIMER_Exists( other, "coverPeek" )
			&& (!TIMER_Exists( other, "coverReturn" ) || TIMER_Exists( other, "saberEscape" ))
			&& DistanceSquared( self->currentOrigin, other->currentOrigin ) <= 512*512 )
			return other;
	}
	return NULL;
}

static gentity_t *ST_FindSupport( gentity_t *self, AIGroupInfo_t *group, const vec3_t known )
{
	for ( int i = 0; i < group->numGroup; ++i )
	{
		gentity_t *other = &g_entities[group->member[i].number];
		if ( other != self && AI_ValidateTacticalMember( group, other ) && other->enemy == self->enemy
			&& !other->NPC->tacticRole && ST_ReadySupport( other, known )
			&& DistanceSquared( self->currentOrigin, other->currentOrigin ) <= 512*512
			&& DistanceSquared( other->currentOrigin, known ) >= 128*128
			&& NAV::InSameRegion( self, other ) && G_ClearLOS( other, other->enemy ) )
			return other;
	}
	return NULL;
}

static void ST_CoverRetreat( gentity_t *support, gentity_t *mover )
{
	ST_AssignTactic( support, 5, -1, mover->NPC->tacticThreat, support->currentOrigin );
	support->NPC->tacticDeadline = Q_min( level.time+6000, mover->NPC->tacticDeadline );
	TIMER_Set( support, "coverSupport", 6000 );
	Debug_Printf( debugNPCAI, DEBUG_LEVEL_INFO, "squad event=retreat_support ent=%d mover=%d\n", support->s.number, mover->s.number );
}

static void ST_PressureDecision( gentity_t *self, const char *reason )
{
	if ( TIMER_Done( self, "pressureDecisionTrace" ) )
	{
		TIMER_Set( self, "pressureDecisionTrace", 500 );
		Debug_Printf( debugNPCAI, DEBUG_LEVEL_INFO, "squad event=pressure_response ent=%d time=%d role=%d reason=%s\n",
			self->s.number, level.time, self->NPC->tacticRole, reason );
	}
}

static qboolean ST_PressureResponse( gentity_t *self )
{
	auto *info = self->NPC;
	bool held = !(info->scriptFlags & SCF_CHASE_ENEMIES)
		|| (info->scriptFlags & (SCF_DONT_FLEE|SCF_CROUCHED|SCF_WALKING));
	bool overrideHold = held && g_squadPressureOverrides->integer;
	bool damagePressure = !TIMER_Done( self, "underFire" );
	if ( TIMER_Done( self, "incomingFire" ) && TIMER_Done( self, "coverRelocate" )
		&& !damagePressure )
		return qfalse;
	AIGroupInfo_t *group = info->group;
	if ( !d_squadTactics->integer || !AI_ValidateTacticalMember( group, self ) || self->enemy != group->enemy
		|| (held && !overrideHold) || (info->scriptFlags & SCF_FIRE_WEAPON)
		|| !TIMER_Done( self, "flee" ) )
	{
		ST_PressureDecision( self, "restricted" );
		return qfalse;
	}
	if ( info->goalEntity && info->goalEntity != self->enemy
		&& (info->goalEntity != info->tempGoal || info->goalEntity->s.eType == ET_ITEM
			|| (info->goalEntity->enemy && info->goalEntity->enemy->s.eType == ET_ITEM)
			|| (info->tacticRole && !VectorCompare( info->goalEntity->currentOrigin, info->tacticGoal ))) )
	{
		ST_PressureDecision( self, "external_goal" );
		return qfalse;
	}
	if ( TIMER_Exists( self, "coverPair" ) && ST_PeekCover( self ) && info->tacticRole )
	{
		ST_PressureDecision( self, "peek_or_hold" );
		return qtrue;
	}
	if ( info->tacticRole == 1 && (!TIMER_Exists( self, "coverReturn" ) || TIMER_Exists( self, "saberEscape" ))
		&& !TIMER_Exists( self, "coverPeek" ) )
	{
		vec3_t threat;
		if ( TIMER_Done( self, "saberPressure" ) || !ST_GetLastSeenPosition( self, threat )
			|| DistanceSquared( info->tacticGoal, threat ) >= 128*128 )
		{
			ST_PressureDecision( self, "already_retreating" );
			return qfalse;
		}
	}
	vec3_t known;
	if ( !ST_GetLastSeenPosition( self, known ) )
	{
		ST_PressureDecision( self, "no_record" );
		return qtrue;
	}
	if ( (info->tacticRole == 0 || (info->tacticRole == 2 && !TIMER_Exists( self, "coverPeek" )))
		&& DistanceSquared( self->currentOrigin, known ) >= 128*128
		&& ST_CoverProtects( self, self->currentOrigin, known, 0 ) )
	{
		TIMER_Set( self, "duck", 1500 );
		if ( info->tacticRole == 2 )
			info->tacticDeadline = Q_max( info->tacticDeadline, level.time+1500 );
		else if ( info->goalEntity )
		{
			info->goalEntity = NULL;
			NAV::ClearPath( self );
			info->aiFlags &= ~NPCAI_TOUCHED_GOAL;
		}
		ST_PressureDecision( self, "concealed" );
		return qtrue;
	}
	if ( !TIMER_Done( self, "pressureSearch" ) )
	{
		ST_PressureDecision( self, "search_backoff" );
		return qtrue;
	}
	TIMER_Set( self, "pressureSearch", 750 );
	// Release offensive roles before assigning a retreat. Keep a flank only with a replacement supporter.
	bool flankSupport = info->tacticRole == 5 && !TIMER_Exists( self, "coverSupport" );
	ST_ClearTactic( self, "pressure" );
	if ( flankSupport )
	{
		for ( int i = 0; i < group->numGroup; ++i )
		{
			gentity_t *flanker = &g_entities[group->member[i].number];
			if ( !flanker->NPC || (flanker->NPC->tacticRole != 3 && flanker->NPC->tacticRole != 4) )
				continue;
			gentity_t *replacement = ST_FindSupport( self, group, flanker->NPC->tacticThreat );
			if ( replacement )
			{
				ST_AssignTactic( replacement, 5, -1, flanker->NPC->tacticThreat, replacement->currentOrigin );
				Debug_Printf( debugNPCAI, DEBUG_LEVEL_INFO, "squad event=support_handoff ent=%d replacement=%d mover=%d\n", self->s.number, replacement->s.number, flanker->s.number );
			}
			else
				ST_ClearTactic( flanker, "support_pressure" );
			break;
		}
	}
	vec3_t goal;
	int cp = -1;
	bool escape = false;
	bool saberClose = !TIMER_Done( self, "saberPressure" );
	if ( (saberClose && DistanceSquared( self->currentOrigin, known ) < 128*128)
		|| !ST_CyclePosition( self, known, qtrue, goal, cp, qfalse, NULL, qfalse ) )
	{
		float distance = Distance( self->currentOrigin, known );
		float retreatDistance = saberClose ? Q_max( distance+64, Q_min( g_squadSaberPressureRadius->value+32, distance+320 ) ) : distance+64;
		escape = ST_CyclePosition( self, known, qtrue, goal, cp, qfalse, NULL, qfalse, retreatDistance );
		if ( !escape )
		{
			ST_PressureDecision( self, "no_cover" );
			return qtrue;
		}
	}
	...
	{
		ST_PressureDecision( self, "reservation_failed" );
		return qtrue;
	}
	NPC_SetMoveGoal( self, goal, 16, qtrue, cp );
	ST_AssignTactic( self, 1, cp, known, goal );
	if ( overrideHold )
		TIMER_Set( self, "pressureMove", 9000 );
	if ( escape )
	{
		// Finish at a firing position, rather than crouching at an exposed escape point.
		TIMER_Set( self, "coverReturn", 6000 );
		TIMER_Set( self, "saberEscape", 6000 );
		// The direct route was just swept and checked for ground support.
		TIMER_Set( self, "SafeToGoToDURATION", 500 );
	}
	else if ( !overrideHold )
		TIMER_Set( self, "coverCycle", 9000 );
	TIMER_Remove( self, "coverRelocate" );
	ST_Speech( self, SPEECH_COVER, 0 );
	gentity_t *support = ST_FindSupport( self, group, known );
	if ( support )
		ST_CoverRetreat( support, self );
	Debug_Printf( debugNPCAI, DEBUG_LEVEL_INFO, "squad event=pressure_cover ent=%d time=%d cp=%d goal=%.3f,%.3f,%.3f knownposition=%.3f,%.3f,%.3f escape=%d override=%d\n",
		self->s.number, level.time, cp, goal[0], goal[1], goal[2], known[0], known[1], known[2], escape, overrideHold );
	return qtrue;
}
...
static qboolean ST_Tactics( gentity_t *self, AIGroupInfo_t *group )
{
	...
	if ( !AI_ValidateTacticalMember( group, self ) || self->enemy != group->enemy
		|| !TIMER_Done( self, "flee" )
	...
	if ( ST_PressureResponse( self ) )
		return qtrue;
	bool pressureMove = g_squadPressureOverrides->integer && TIMER_Exists( self, "pressureMove" );
	if ( !(info->scriptFlags & SCF_CHASE_ENEMIES) && !pressureMove )
	{
		ST_ClearTactic( self );
		return qfalse;
	}
	...
	for ( int i = 0; i < group->numGroup; i++ )
	{
		...
		if ( ST_ReadySupport( other, info->tacticRole ? info->tacticThreat : group->enemyLastSeenPos )
			&& !TIMER_Exists( other, "coverSupport" )
	...
	if ( info->tacticRole )
	{
		...
		if ( role == 1 && (info->scriptFlags & SCF_DONT_FLEE) && !pressureMove )
		{
		...
		{
			VectorCopy( self->currentOrigin, info->tacticCover );
			TIMER_Set( self, "coverPair", 15000 );
			TIMER_Set( self, "coverPaired", 15000 );
		}
		if ( ST_PeekCover( self ) )
			return qtrue;
		if ( info->tacticRole == 5 && ((TIMER_Exists( self, "coverSupport" ) ? !ST_RetreatingMember( self, group ) : !flanker)
			|| lowHealth || !ST_ReadySupport( self, info->tacticThreat )) )
		{
		...
		else if ( arrived )
		{
			...
			else
			{
				...
				if ( TIMER_Exists( self, "coverPeek" ) )
				{
					info->tacticDeadline = level.time + Q_irand( 1000, 1800 );
					Debug_Printf( debugNPCAI, DEBUG_LEVEL_INFO, "squad event=peek_fire ent=%d\n", self->s.number );
				}
			}
		}
	...
	// Snipers retain their own firing controller outside pressure retreats.
	if ( self->client->ps.weapon == WP_DISRUPTOR )
		return qfalse;
	...
	if ( !TIMER_Done( self, "regroupRetry" ) )
		return qtrue;
	gentity_t *retreat = ST_RetreatingMember( self, group );
	if ( retreat && ST_ReadySupport( self, retreat->NPC->tacticThreat ) && G_ClearLOS( self, self->enemy ) )
	{
		ST_CoverRetreat( self, retreat );
		return qtrue;
	}
		...
		&& !lowHealth && !(info->scriptFlags & (SCF_DONT_FLEE|SCF_DONT_FIRE|SCF_FIRE_WEAPON)) )
	{
		...
		if ( ST_CyclePosition( self, group->enemyLastSeenPos, qtrue, goal, cp ) )
		{
			...
			if ( cp < 0 || NPC_SetCombatPoint( cp ) )
			{
				...
				TIMER_Set( self, "coverCycle", 9000 );
				ST_Speech( self, SPEECH_COVER, 0 );
				gentity_t *coverSupport = ST_FindSupport( self, group, group->enemyLastSeenPos );
				if ( coverSupport )
					ST_CoverRetreat( coverSupport, self );
...
void ST_Commander( void )
{
	...
	{//dissolve the group
		...
		for ( i = 0; i < group->numGroup; i++ )
		{
			...
			if ( member->enemy != group->enemy || !AI_ValidateTacticalMember( group, member ) )
			{
				continue;
			}
	...
	for ( i = curMemberNum; i < lastMemberNum; i++ )
	{
		...
		SetNPCGlobals( member );
		if ( !AI_ValidateTacticalMember( group, member ) )
			continue;
		if ( ST_CheckGrenade() )
		{
		...
		if ( ST_Tactics( member, group ) )
			continue;
		if ( member->client->ps.weapon == WP_DISRUPTOR )
			continue;
...
void NPC_BSST_Attack( void )
{
	...
	ST_SaberPressure( NPC, enemyLOS );
	ST_PressureResponse( NPC );
	ST_CheckMoveState();
	...
	if ( !doMove )
	{
		if (NPC->client->NPC_class != CLASS_ASSASSIN_DROID)
		{
			if ( !TIMER_Exists( NPC, "coverPeek" ) && (!TIMER_Done( NPC, "duck" ) || (NPCInfo->tacticRole == 2
				&& (NPCInfo->tacticCP >= 0 || TIMER_Exists( NPC, "coverCycle" )))) )
			{
				ucmd.upmove = -127;
			}
	...
	if ( (NPCInfo->scriptFlags & SCF_DONT_FIRE)
		|| (TIMER_Exists( NPC, "coverPeek" ) && NPCInfo->tacticRole != 2)
		|| (NPCInfo->tacticRole == 2 && TIMER_Exists( NPC, "coverCycle" ) && !TIMER_Exists( NPC, "coverPeek" )) )
	{
		shoot = qfalse;
	}
		...
		if ( TIMER_Done( NPC, "attackDelay" ) )
		{
			if( !(NPCInfo->scriptFlags & SCF_FIRE_WEAPON) ) // we've already fired, no need to do it again here
			{
				...
				if ( ucmd.buttons & (BUTTON_ATTACK|BUTTON_ALT_ATTACK) )
					Debug_Printf( debugNPCAI, DEBUG_LEVEL_DETAIL, "squad event=fire_attempt ent=%d time=%d role=%d peek=%d\n", NPC->s.number, level.time, NPCInfo->tacticRole, TIMER_Exists( NPC, "coverPeek" ) );
			}
			//NASTY
			if ( NPC->s.weapon == WP_ROCKET_LAUNCHER )
			{
	...
	if ( TIMER_Exists( NPC, "coverPeek" ) && NPCInfo->tacticRole == 2 && !enemyCS )
	{
		if ( !TIMER_Exists( NPC, "peekBlocked" ) )
			TIMER_Set( NPC, "peekBlocked", 500 );
	}
	else
		TIMER_Remove( NPC, "peekBlocked" );
	if ( d_squadTactics->integer && !NPCInfo->tacticRole && enemyLOS
		&& NPCInfo->group && NPCInfo->group->enemy == NPC->enemy
		&& AI_ValidateTacticalMember( NPCInfo->group, NPC )
		&& (NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)
...
{
	if ( !d_squadTactics->integer || !NPC->enemy || !NPC->enemy->client )
		return qfalse;
	AI_GetGroup( NPC );
	if ( !AI_ValidateTacticalMember( NPCInfo->group, NPC ) || NPCInfo->group->enemy != NPC->enemy )
		return qfalse;
	...
	ST_SaberPressure( NPC, visible );
	ST_PressureResponse( NPC );
	if ( !NPCInfo->tacticRole )
		return qfalse;
	TIMER_Remove( NPC, "coverCycle" );
	NPC_BSST_Attack();
	return qtrue;
}
